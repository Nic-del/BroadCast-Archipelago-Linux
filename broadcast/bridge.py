...
if platform.system() == "Linux":
    try:
        ...
        p.nice(10) # Set a lower priority on Linux (0 to 19, 10 is balanced)
    except:
        pass
...
def kill_port(port):
    """Forcefully close any process using the specified port (cross-platform)."""
    import psutil
            ...
            for conn in proc.net_connections(kind='inet'):
                if conn.laddr.port == port:
                    if proc.pid != os.getpid():
                        print(f"Closing existing process {proc.info['name']} on port {port} (PID {proc.pid})...")
                        proc.kill()
