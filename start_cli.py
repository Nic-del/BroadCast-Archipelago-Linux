...
def main():
    ...
    try:
        ...
        if settings["mode"] == "obs":
            ...
            procs.append(subprocess.Popen(["npx", "vite", "--no-open"], cwd="broadcast-app"))
        ...
        if not os.path.exists(dist_path) or not os.path.exists(os.path.join(dist_path, "index.html")):
            ...
            subprocess.run(["npm", "run", "build"], cwd="broadcast-app")
        ...
        procs.append(subprocess.Popen(["npm", "run", "overlay"], cwd="broadcast-app"))
        ...
        # Clean up any leftover node/electron processes
