...
def get_monitors():
    ...
    try:
        ...
        # Very basic fallback for standard primary screen if screeninfo fails
...
class BroadcastLauncherApp:
    ...
    def launch_background_tasks(self):
        # On Linux, we just use the standard command names
        npm_cmd = "npm"
        npx_cmd = "npx"
        
        # Start Dev Server for OBS Page (if needed, but without opening browser)
        self.procs.append(subprocess.Popen([npx_cmd, "vite", "--no-open"], cwd="broadcast-app"))
        
        # Determine bridge mode (obs mode uses 'all' filtering)
        bridge_mode = "all" if self.settings["mode"] in ["all", "obs"] else "personal"
        
        # Use sys.executable to ensure we use the same Python that launched this script
        bridge_cmd = [sys.executable, "-u", os.path.join("broadcast", "bridge.py"), "--server", self.settings["server"], "--slot", self.settings["slot"], "--mode", bridge_mode]
        ...
        self.procs.append(subprocess.Popen([npm_cmd, "run", "overlay"], cwd="broadcast-app"))
    ...
    def stop_system(self):
        ...
        # Kill tracked processes using psutil (cross-platform way to kill process trees)
        for p in self.procs:
            try:
        ...
        # Extra safety for any remaining orphans
        for proc in psutil.process_iter(['name']):
            try:
                # Removed .exe as we are on Linux
                if proc.info['name'] in ['node', 'electron']:
                    # Only kill if it seems to be ours (optional: check CWD or cmdline)
                    proc.kill()
            except: pass
